    ...
    def run(self, agents):
        ...
        while len(remaining_agents) > 0:
            ...
            if len(remaining_agents) == 2: # switch to "I cut, you choose"
                ...
                logging.debug("Two agents, %d and %d, remain" % (agent1.id, agent2.id))
                k = agent1.cut_halves(i, j)
                logging.debug("Agent %s cuts (%d, %d) at %d" % (agent1.id, i, j, k))
                a, b = agent2.choose_half(i, k, j)
                c, d = (k, j) if a == i else (i, k)
                logging.debug("Agent %s chooses (%d, %d)" % (agent2.id, a, b))
                logging.debug("Agent %s receives (%d, %d)" % (agent2.id, c, d))
            ...
            for agent in remaining_agents:
                ...
                if dim is not None: # first agent should always diminish
                    logging.debug("Agent %s diminishes (%d, %d) to (%d, %d)" %
                                 (agent.id, i, j, i, dim))
                    last_diminisher = agent
                    j = dim
            remaining_agents.remove(last_diminisher)
            allocations[last_diminisher.id] = range(i, j)
            logging.debug("Agent %s is the last diminisher and receives (%d, %d)" %
